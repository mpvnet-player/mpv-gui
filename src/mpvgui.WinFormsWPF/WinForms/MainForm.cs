...
public partial class MainForm : Form
{
    ...
    void CM_Popup(object sender, EventArgs e) => ShowCursor();
    ...
    protected override void WndProc(ref Message m)
    {
        switch (m.Msg)
        {
            ...
            case 0x200: // WM_MOUSEMOVE
                ...
                if (IsCursorPosDifferent(LastCursorPosition))
                    ShowCursor();
                break;
        ...
        if (m.Msg == TaskbarButtonCreatedMessage && Player.TaskbarProgress)
        {
    ...
    {
        if (Player.TaskbarProgress && Taskbar != null)
            Taskbar.SetValue(Player.GetPropertyDouble("time-pos", false), Player.Duration.TotalSeconds);
    }
    ...
    void Player_Pause()
    {
        if (Taskbar != null && Player.TaskbarProgress)
        {
            if (Player.Paused)
